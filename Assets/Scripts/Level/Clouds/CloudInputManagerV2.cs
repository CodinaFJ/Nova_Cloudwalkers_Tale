...
public class CloudInputManagerV2 : MonoBehaviour
{
    ...
    void Awake()
    {
        Debug.Log("CloudInputManager Created");
    }
    ...
    IEnumerator MoveCloudOneStep()
    {
        ...
        {    
            while (currentTime < cloudMovementTime)
            {
                ...
                easedMovement = NormalizedEasedMovement(currentTime/cloudMovementTime);

                cloudsParents[item-1].transform.position = Vector3.Lerp(cloudInitialPosition, cloudFinalPosition, Mathf.Clamp(currentTime / cloudMovementTime, 0f, 1f));
                //WobbleCloud(currentTime);
        ...
        {
            //cloudsParents[item-1].transform.position = cloudInitialPosition + (cloudFinalPosition - cloudInitialPosition) * easedMovement;
            LTCloudMovement = LeanTween.move(cloudsParents[item-1], cloudFinalPosition, cloudMovementTime).setEaseOutElastic();
        }
        ...
        /*if(mouseMovements.Length > 0)
        {
            if(mouseMovements[0] == movementDone)
            {
                moveCloud();
